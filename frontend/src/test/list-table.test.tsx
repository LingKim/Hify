...
describe("ListTable", () => {
  ...
  it("collapses row actions after three actions", async () => {
    const api = vi.fn(async (_params: ListRequestParams<DemoQuery>) =>
      createPageResult([{ id: 1, name: "Alpha", status: "enabled" }]),
    );
    ...
    expect(await screen.findByText("Alpha")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "操作一" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "更多操作" })).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "操作二" })).not.toBeInTheDocument();
  });
});
